...
def generate_launch_description():
    ...
    pkg_cpr_agriculture_gazebo = get_package_share_directory('cpr_agriculture_gazebo')
    ...
    world_name_arg = DeclareLaunchArgument(
        'world_name',
        default_value=os.path.join(pkg_cpr_agriculture_gazebo, 'worlds', 'actually_empty_world.world'),
        description='Path to world file'
    )

    # Process agriculture geometry URDF
    agriculture_geom_xacro = os.path.join(pkg_cpr_agriculture_gazebo, 'urdf', 'agriculture_geometry.urdf.xacro')
    agriculture_geom_urdf = Command([
        FindExecutable(name='xacro'), ' ', agriculture_geom_xacro
    ])
    ...
    spawn_agriculture_geometry = ExecuteProcess(
        cmd=[
            'ros2', 'run', 'ros_gz_sim', 'create',
            '-topic', 'robot_description',
            '-name', 'agriculture_geometry',
