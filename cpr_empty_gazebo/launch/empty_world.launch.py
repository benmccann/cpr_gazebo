...
def generate_launch_description():
    ...
    robot_z_arg = DeclareLaunchArgument('robot_z', default_value='2.0')
    ...
    world_name_arg = DeclareLaunchArgument(
        'world_name',
        default_value=os.path.join(pkg_cpr_empty_gazebo, 'worlds', 'actually_empty_world.world'),
        description='Path to world file'
    )

    # Process empty geometry URDF
    empty_geom_xacro = os.path.join(pkg_cpr_empty_gazebo, 'urdf', 'empty_geometry.urdf.xacro')
    empty_geom_urdf = Command([
        FindExecutable(name='xacro'), ' ', empty_geom_xacro
    ])
    ...
    spawn_empty_geometry = ExecuteProcess(
        cmd=[
    ...
    platform_spawn = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            PathJoinSubstitution([
                pkg_cpr_empty_gazebo,
                'launch',
                [LaunchConfiguration('platform'), '_spawn.launch.py']
            ])
        ]),
        launch_arguments={
