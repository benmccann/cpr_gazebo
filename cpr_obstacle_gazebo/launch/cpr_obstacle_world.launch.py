...
def generate_launch_description():
    ...
    pkg_cpr_obstacle_gazebo = get_package_share_directory('cpr_obstacle_gazebo')
    ...
    world_name_arg = DeclareLaunchArgument(
        'world_name',
        default_value=os.path.join(pkg_cpr_obstacle_gazebo, 'worlds', 'actually_empty_world.world'),
        description='Path to world file'
    )

    # Process cpr_obstacle geometry URDF
    cpr_obstacle_geom_xacro = os.path.join(pkg_cpr_obstacle_gazebo, 'urdf', 'obstacle_geometry.urdf.xacro')
    cpr_obstacle_geom_urdf = Command([
        FindExecutable(name='xacro'), ' ', cpr_obstacle_geom_xacro
    ])
    ...
    # Spawn cpr_obstacle world geometry
    spawn_cpr_obstacle_geometry = ExecuteProcess(
        cmd=[
            'ros2', 'run', 'ros_gz_sim', 'create',
            '-topic', 'robot_description',
            '-name', 'cpr_obstacle_geometry',
