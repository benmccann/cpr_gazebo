...
def generate_launch_description():
    ...
    pkg_cpr_office_gazebo = get_package_share_directory('cpr_office_gazebo')
    ...
    world_name_arg = DeclareLaunchArgument(
        'world_name',
        default_value=os.path.join(pkg_cpr_office_gazebo, 'worlds', 'office_construction_world.world'),
        description='Path to world file'
    )

    # Process office_construction geometry URDF
    office_construction_geom_xacro = os.path.join(pkg_cpr_office_gazebo, 'urdf', 'office_construction_geometry.urdf.xacro')
    office_construction_geom_urdf = Command([
        FindExecutable(name='xacro'), ' ', office_construction_geom_xacro
    ])
    ...
    # Spawn office_construction world geometry
    spawn_office_construction_geometry = ExecuteProcess(
        cmd=[
            'ros2', 'run', 'ros_gz_sim', 'create',
            '-topic', 'robot_description',
            '-name', 'office_construction_geometry',
    ...
    return LaunchDescription([
        ...
        gazebo_server,
        gazebo_client,
        spawn_office_construction_geometry,
        platform_spawn
    ])
