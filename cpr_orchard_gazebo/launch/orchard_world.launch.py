...
def generate_launch_description():
    ...
    world_name_arg = DeclareLaunchArgument(
        'world_name',
        default_value=os.path.join(pkg_cpr_orchard_gazebo, 'worlds', 'actually_empty_world.world'),
        description='Path to world file'
    )

    # Process orchard geometry URDF
    orchard_geom_xacro = os.path.join(pkg_cpr_orchard_gazebo, 'urdf', 'orchard_geometry.urdf.xacro')
    orchard_geom_urdf = Command([
        FindExecutable(name='xacro'), ' ', orchard_geom_xacro
    ])
    ...
    spawn_orchard_geometry = ExecuteProcess(
        cmd=[
            'ros2', 'run', 'ros_gz_sim', 'create',
            '-topic', 'robot_description',
            '-name', 'orchard_geometry',
